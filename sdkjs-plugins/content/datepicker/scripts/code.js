...
// FIXED: Removed unnecessary 500ms timeout
window.Asc.plugin.init = function () {
  if (this.executeMethod) window.pluginAPI = this;
...
function initializeDatePicker() {
  ...
  // FIXED: Removed unnecessary debounce timeout - direct update is fine for select dropdown
  formatSelect.addEventListener("change", () => {
    calendar.updateInput();
  });
  ...
  // FIXED: Simplified - callCommand is synchronous, just needs a small delay for UI update
  insertBtn.addEventListener("click", () => {
    const selectedDate = calendar.getDate();
    if (!selectedDate) {
      return;
